...
class StudentChatService:
    """Manages active WebSockets channels grouped by course/session and handles chat history."""
    ...
    async def connect_room(cls, websocket: WebSocket, room_key: str) -> None:
        """Registers a client connection to a specific channel room."""
        await websocket.accept()
        if room_key not in cls.active_rooms:
            cls.active_rooms[room_key] = set()
        cls.active_rooms[room_key].add(websocket)
        logger.info(f"Client registered to channel {room_key}", count=len(cls.active_rooms[room_key]))

    @classmethod
    def disconnect_room(cls, websocket: WebSocket, room_key: str) -> None:
        """Removes a client connection from a specific channel room."""
        ...
        logger.info(f"Client unregistered from channel {room_key}")
